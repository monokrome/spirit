...
impl AudioGenerator {
    ...
    fn generate_singing_bowl(&self, frequency: f64, duration_secs: f64) -> Vec<f64> {
        ...
        for i in 0..num_samples {
            ...
            let fundamental =
                (2.0 * PI * frequency * t).sin() * (1.0 + 0.1 * (2.0 * PI * beat_freq * t).sin());
    ...
    fn generate_frequency_sweep_file(&self, start: f64, end: f64) -> Result<(), hound::Error> {
        ...
        println!(
            "\n=== Generating Frequency Sweep {} Hz -> {} Hz ===",
            start, end
        );
        let samples = self.generate_frequency_sweep(start, end, self.duration);
        let path = self
            .output_dir
            .join(format!("sweep_{:.0}hz_to_{:.0}hz.wav", start, end));
        self.save_mono_wav(&path, &samples)?;
    ...
    fn generate_drone_file(&self, frequencies: &[f64]) -> Result<(), hound::Error> {
        ...
        let path = self
            .output_dir
            .join(format!("drone_{}.wav", freq_str.join("_")));
        self.save_mono_wav(&path, &samples)?;
    ...
    fn generate_dimension_set(&self) -> Result<(), hound::Error> {
        ...
        for freq_info in DIMENSION_FREQUENCIES {
            ...
            if freq_info.hz < 20.0 {
                let samples = self.generate_binaural_beat(200.0, freq_info.hz, self.duration);
                let path = dir.join(format!(
                    "dimension_{}_{:.0}hz_binaural.wav",
                    freq_info.name, freq_info.hz
                ));
                self.save_stereo_wav(&path, &samples)?;
            } else {
    ...
    fn generate_cognitive_set(&self) -> Result<(), hound::Error> {
        ...
        for freq_info in COGNITIVE_FREQUENCIES {
            ...
            if freq_info.hz < 20.0 {
                let samples = self.generate_binaural_beat(200.0, freq_info.hz, self.duration);
                let path = dir.join(format!(
                    "cognitive_{}_{:.0}hz_binaural.wav",
                    freq_info.name, freq_info.hz
                ));
                self.save_stereo_wav(&path, &samples)?;
                ...
                let path = dir.join(format!(
                    "cognitive_{}_{:.0}hz_isochronic.wav",
                    freq_info.name, freq_info.hz
                ));
                self.save_mono_wav(&path, &samples)?;
            } else {
    ...
    fn generate_circadian_set(&self) -> Result<(), hound::Error> {
        ...
        for freq_info in CIRCADIAN_FREQUENCIES {
            ...
            if freq_info.hz < 20.0 {
                let samples = self.generate_binaural_beat(100.0, freq_info.hz, self.duration);
                let path = dir.join(format!(
                    "circadian_{}_{:.1}hz_binaural.wav",
                    freq_info.name, freq_info.hz
                ));
                self.save_stereo_wav(&path, &samples)?;
            } else {
    ...
    fn generate_kundalini_set(&self) -> Result<(), hound::Error> {
        ...
        for freq_info in KUNDALINI_FREQUENCIES {
            ...
            if freq_info.hz < 20.0 {
                let samples = self.generate_binaural_beat(200.0, freq_info.hz, self.duration);
                let path = dir.join(format!(
                    "kundalini_{}_{:.0}hz_binaural.wav",
                    freq_info.name, freq_info.hz
                ));
                self.save_stereo_wav(&path, &samples)?;
            } else {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Commands::Custom { frequency, mode } => {
            ...
            match mode {
                GenerationMode::Sine => {
                    ...
                    let path = gen
                        .output_dir
                        .join(format!("custom_{:.1}hz_sine.wav", frequency));
                    gen.save_mono_wav(&path, &samples)?;
                }
                GenerationMode::Binaural => {
                    ...
                    let path = gen
                        .output_dir
                        .join(format!("custom_{:.1}hz_binaural.wav", frequency));
                ...
                GenerationMode::Isochronic => {
                    ...
                    let path = gen
                        .output_dir
                        .join(format!("custom_{:.1}hz_isochronic.wav", frequency));
                    gen.save_mono_wav(&path, &samples)?;
                }
            }
        }
        ...
        Commands::Layer { frequencies } => {
            ...
            let path = gen
                .output_dir
                .join(format!("layered_{}.wav", freq_str.join("_")));
            gen.save_mono_wav(&path, &samples)?;
        }
    }
...
mod tests {
    ...
    // -------------------------------------------------------------------------
    // AudioGenerator Initialization Tests
    // -------------------------------------------------------------------------

    #[test]
    fn test_audio_generator_new() {
        let config = AudioConfig {
            sample_rate: 48000,
            bit_depth: 24,
        };
        let gen = AudioGenerator::new(PathBuf::from("/tmp"), 10.0, config);

        assert_eq!(gen.config.sample_rate, 48000);
        assert_eq!(gen.config.bit_depth, 24);
        assert_eq!(gen.duration, 10.0);
    ...
    #[test]
    fn test_convert_sample_i16_max() {
        let result = convert_sample_i16(1.0);
        assert_eq!(result, i16::MAX);
    }

    #[test]
    fn test_convert_sample_i16_min() {
        let result = convert_sample_i16(-1.0);
        // Uses symmetric scaling: -1.0 * i16::MAX = -32767
        assert_eq!(result, -i16::MAX);
    }

    #[test]
    fn test_convert_sample_i16_zero() {
        let result = convert_sample_i16(0.0);
        assert_eq!(result, 0);
    }

    #[test]
    fn test_convert_sample_i32_max() {
        let result = convert_sample_i32(1.0);
        assert_eq!(result, i32::MAX);
    }

    #[test]
    fn test_convert_sample_i32_min() {
        let result = convert_sample_i32(-1.0);
        // Uses symmetric scaling: -1.0 * i32::MAX = -2147483647
        assert_eq!(result, -i32::MAX);
    }

    #[test]
    fn test_convert_sample_i32_zero() {
        let result = convert_sample_i32(0.0);
        assert_eq!(result, 0);
    }

    #[test]
    fn test_convert_sample_i32_24bit_max() {
        let result = convert_sample_i32_24bit(1.0);
        assert_eq!(result, 8388607); // 2^23 - 1
    }

    #[test]
    fn test_convert_sample_i32_24bit_min() {
        let result = convert_sample_i32_24bit(-1.0);
        assert_eq!(result, -8388607);
    }

    #[test]
    fn test_convert_sample_i32_24bit_zero() {
        let result = convert_sample_i32_24bit(0.0);
        assert_eq!(result, 0);
    }
    ...
    fn create_test_generator() -> AudioGenerator {
        let config = AudioConfig {
            sample_rate: 44100,
            bit_depth: 16,
        };
        AudioGenerator::new(PathBuf::from("/tmp"), 1.0, config)
    }

    #[test]
    fn test_generate_sine_wave_length() {
        let gen = create_test_generator();
        let samples = gen.generate_sine_wave(440.0, 1.0);
        assert_eq!(samples.len(), 44100); // 1 second at 44100 Hz
    }

    #[test]
    fn test_generate_sine_wave_length_half_second() {
        let gen = create_test_generator();
        let samples = gen.generate_sine_wave(440.0, 0.5);
        assert_eq!(samples.len(), 22050); // 0.5 seconds at 44100 Hz
    }

    #[test]
    fn test_generate_sine_wave_amplitude_range() {
        let gen = create_test_generator();
        let samples = gen.generate_sine_wave(440.0, 1.0);
    ...
    #[test]
    fn test_generate_binaural_beat_length() {
        let gen = create_test_generator();
        let samples = gen.generate_binaural_beat(200.0, 10.0, 1.0);
        assert_eq!(samples.len(), 44100);
    }

    #[test]
    fn test_generate_binaural_beat_stereo() {
        let gen = create_test_generator();
        let samples = gen.generate_binaural_beat(200.0, 10.0, 0.1);

        // Verify we have stereo samples (left and right channels)
        for sample in &samples {
    ...
    #[test]
    fn test_generate_binaural_beat_amplitude_range() {
        let gen = create_test_generator();
        let samples = gen.generate_binaural_beat(200.0, 10.0, 1.0);
    ...
    #[test]
    fn test_generate_isochronic_tone_length() {
        let gen = create_test_generator();
        let samples = gen.generate_isochronic_tone(200.0, 10.0, 1.0);
        assert_eq!(samples.len(), 44100);
    }

    #[test]
    fn test_generate_isochronic_tone_amplitude_range() {
        let gen = create_test_generator();
        let samples = gen.generate_isochronic_tone(200.0, 10.0, 1.0);
    ...
    #[test]
    fn test_generate_singing_bowl_length() {
        let gen = create_test_generator();
        let samples = gen.generate_singing_bowl(432.0, 2.0);
        assert_eq!(samples.len(), 88200); // 2 seconds at 44100 Hz
    }

    #[test]
    fn test_generate_singing_bowl_amplitude_range() {
        let gen = create_test_generator();
        let samples = gen.generate_singing_bowl(432.0, 1.0);
    ...
    #[test]
    fn test_generate_om_tone_length() {
        let gen = create_test_generator();
        let samples = gen.generate_om_tone(1.0);
        assert_eq!(samples.len(), 44100);
    }

    #[test]
    fn test_generate_om_tone_amplitude_range() {
        let gen = create_test_generator();
        let samples = gen.generate_om_tone(1.0);
    ...
    #[test]
    fn test_generate_layered_frequencies_length() {
        let gen = create_test_generator();
        let frequencies = vec![100.0, 200.0, 300.0];
        let samples = gen.generate_layered_frequencies(&frequencies, 1.0);
        assert_eq!(samples.len(), 44100);
    }

    #[test]
    fn test_generate_layered_frequencies_amplitude_range() {
        let gen = create_test_generator();
        let frequencies = vec![100.0, 200.0, 300.0];
        let samples = gen.generate_layered_frequencies(&frequencies, 1.0);
    ...
    fn test_sine_wave_frequency_accuracy() {
        ...
        let gen = AudioGenerator::new(PathBuf::from("/tmp"), 1.0, config);

        let frequency = 440.0;
        let samples = gen.generate_sine_wave(frequency, 1.0);
        ...
        // For a 440 Hz sine wave over 1 second, we expect ~440 zero crossings
        // Allow 1% tolerance
        let expected = frequency as i32;
        let tolerance = (expected as f64 * 0.01) as i32;
        assert!(
            (zero_crossings - expected).abs() <= tolerance,
    ...
    fn test_sine_wave_starts_at_zero() {
        ...
        // sin(0) = 0, so first sample should be very close to 0
        assert!(
            samples[0].abs() < 0.01,
    ...
    #[test]
    fn test_binaural_beat_frequency_difference() {
        let config = AudioConfig {
            sample_rate: 44100,
            bit_depth: 16,
        };
        ...
        let base_freq = 200.0;
        let beat_freq = 10.0;
        let samples = gen.generate_binaural_beat(base_freq, beat_freq, 1.0);

        // Count zero crossings for left channel (should be ~200 Hz)
        ...
        // Count zero crossings for right channel (should be ~210 Hz)
        let mut right_crossings = 0;
        for i in 1..samples.len() {
            if samples[i - 1][1] >= 0.0 && samples[i][1] < 0.0 {
                right_crossings += 1;
            }
        }

        // Verify left is ~200 Hz and right is ~210 Hz
        let tolerance: i32 = 3;
        assert!(
            (left_crossings as i32 - 200).abs() <= tolerance,
            "Left channel expected ~200 Hz, got {} Hz",
            left_crossings
        );
        assert!(
            (right_crossings as i32 - 210).abs() <= tolerance,
            "Right channel expected ~210 Hz, got {} Hz",
            right_crossings
        );
    }
    ...
    fn test_different_sample_rates() {
        for &rate in &[22050u32, 44100, 48000, 96000] {
            ...
            assert_eq!(
                samples.len(),
                rate as usize,
                "Sample count mismatch for {} Hz sample rate",
                rate
            );
        }
    }
    ...
    #[test]
    fn test_high_sample_rate_frequency_accuracy() {
        let config = AudioConfig {
            sample_rate: 96000,
            bit_depth: 24,
        };
        ...
        assert!(
            (zero_crossings - expected).abs() <= tolerance,
            "Expected ~{} zero crossings at 96kHz, got {}",
            expected,
            zero_crossings
        );
    }
